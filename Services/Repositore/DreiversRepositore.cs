...
namespace EL.BlackList.API.Services.Repositore
{
    public class DriversRepositore : IDriversRepositore
    {
        ...
        public Drivers? GetDriversId(int id)
        {
            if (id > 0)
            {
                var result = _context.Drivers.Include(fb => fb.FeedBacks).ThenInclude(tp => tp.TaxiPools).Include(fb => fb.FeedBacks).ThenInclude(c => c.City).FirstOrDefault(d => d.DriverId == id);
                return result;
            }
            return null;
        }
